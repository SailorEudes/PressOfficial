<div
    ...
    <div class="absolute inset-0 z-30 hidden sm:block" :class="{ 'bottom-0': !open }">
        ...
        <div class="inset-0 z-40 transition duration-200 ease-out" :class="{ 'absolute h-14': open, 'relative h-10 -mt-10': !open }">
            <div class="w-full h-full border-t border-blue-500 bg-gradient-to-r from-gray-500 via-blue-500 to-purple-600" :class="{ 'overflow-hidden': open }">
                <div class="flex justify-between w-full h-full">
                    <div class="flex h-full">
                        ...
                        <div @click="open=true; url='/docs'; active='docs';" class="flex items-center justify-center h-full text-xs leading-none text-blue-100 border-l border-blue-500 cursor-pointer hover:bg-gray-600" :class="{ 'px-3': !open, 'px-5': open, 'bg-gray-600': active == 'docs' && open, 'bg-gray-500': active != 'docs' }">
                            Documentation
                        </div>
                        @if(!auth()->guest() && auth()->user()->can('browse_admin'))
                            <div @click="open=true; url='/admin'; active='admin';" class="flex items-center justify-center h-full text-xs leading-none text-blue-100 border-l border-blue-500 cursor-pointer hover:bg-gray-600" :class="{ 'px-3': !open, 'px-5': open, 'bg-gray-600': active == 'admin' && open, 'bg-gray-500': active != 'admin' }">
                                Admin
                            </div>
                        @endif
                    </div>
