<div class="flex flex-col px-10 py-8">
	<form wire:submit.prevent="add">
		...
		<div class="flex justify-end w-full mt-2">
			<button class="flex self-end justify-center w-auto px-4 py-2 mt-5 text-sm font-medium text-white transition duration-150 ease-in-out border border-transparent rounded-md bg-gray-600 hover:bg-gray-500 focus:outline-none focus:border-wave-700 focus:shadow-outline-wave active:bg-gray-700" dusk="update-profile-button">Create New Key</button>
		</div>
	...
	@if(count($keys) > 0)
		...
		<script>
			window.actionClicked = function(action, id, name, key){
				let alpineData = document.getElementById(action + 'ApiKey').__x.$data;
		...
		<!-- BELOW ARE THE MODALS TO VIEW, EDIT, AND DELETE AN API KEY -->
			...
			<div x-data="{ open: @entangle('readModal') }" x-init="
				...
				});" id="viewApiKey" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
			...
			<div x-data="{ open: @entangle('editModal') }" x-init="
				...
				});" id="editApiKey" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
				<div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
					<div x-show="open" x-on:click="open = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 transition-opacity">
						<div class="absolute inset-0 bg-black opacity-50"></div>
					</div>
						...
						<div class="flex flex-col justify-between w-full mt-2">
							<form wire:submit.prevent="edit">
								...
								<p class="flex justify-end mt-5">
									...
									<button type="submit" class="inline-flex justify-center px-4 py-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out border border-transparent rounded-md shadow-sm bg-gray-600 hover:bg-gray-500 focus:outline-none focus:border-wave-700 focus:shadow-outline-wave sm:text-sm sm:leading-5">Update</button>
								</p>
							</form>
						</div>
					</div>
			...
			<div x-data="{ open: @entangle('deleteModal') }" x-init="
				...
				});" id="deleteApiKey" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
				<div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
					<div x-show="open" x-on:click="open = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 transition-opacity">
						<div class="absolute inset-0 bg-black opacity-50"></div>
					</div>
